...
- Demo 10 observable local graduation gates with no permanent unlock
- Phase 1 production contract: frozen scope, non-custodial intent, reconciliation states, and privacy contract

It deliberately has no page scraping, private-key handling, or silent signing. Unknown selectors and unsupported questions remain unknown. Cached familiarity never replaces live verification or authorizes signing. Holder records are not a social graph or contact permission. Network filters are not endorsement, safety, liquidity, or custody claims. Demo 9 does not mint, message, join a real network, or infer understanding. Demo 10 reports only observable local exercise completion; it does not mint or grant membership. Phase 1 freezes Cyprus-1/native QUAI/external-provider scope; provider acceptance is not confirmation, and final success requires independent receipt readback. The extension is useful before it is trusted.
